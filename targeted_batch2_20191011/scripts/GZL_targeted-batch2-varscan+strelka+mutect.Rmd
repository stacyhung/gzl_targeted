...
colnames(varScan.calls) <- c("sample", "chr", "pos", "external_id", "ref", "alt", 
                             "filter", "gt_normal", "gq_normal", "depth_normal", "ref_reads_normal", "var_reads_normal",
                             "allele_freq_normal", "depth4_normal", "gt_tumor", "gq_tumor", "depth_tumor", "ref_reads_tumor",
                             "var_reads_tumor", "allele_freq_tumor", "depth4_tumor", "depth", "somatic", "somatic_status",
...
varScan.calls$tumor_id <- revalue(varScan.calls$tumor_id, tumor_id.to.mappable_tumor_id)

# remove excluded samples
varScan.calls <- filter(varScan.calls, !(varScan.calls$tumor_id %in% samples.to.exclude))
...
keeps <- c("tumor_id", "normal_id", "type", "chr", "pos", "gene", "ref", "alt", "hgvs_cdna", "hgvs_protein",	
           "allele_freq_normal", "allele_freq_tumor",  "depth", "depth4_normal", "depth4_tumor",
           "var_reads_normal", "var_reads_tumor", "effect", "impact", "biotype", "external_id", 
           "somatic_p_value", "somatic_status", "somatic", "gt_tumor")	
...
varScan.calls <- merge(varScan.calls, sample.data, by = "tumor_id", all.x = TRUE)

# apply filters that can be applied to ALL samples
varScan.calls <- filter(varScan.calls, somatic_status == 2 & as.numeric(var_reads_tumor) >= MIN_VAR_READS_TUMOR)
...
varScan.not_pooledN <- filter(varScan.calls, normal_id != "pooledN") # dataset 1
varScan.pooledN <- filter(varScan.calls, normal_id == "pooledN")
...
colnames(mutect.snvs) <- c("sample", "chr", "pos", "external_id", "ref", 
          "alt", "filter", "gt_normal", "normal.var_depth", "normal.base_qual", 
          "depth_normal", "allele_freq_normal", "normal.somatic_status", "gt_tumor", "tumor.var_depth", 
          "tumor.base_qual", "depth_tumor", "allele_freq_tumor", "somatic_status", "somatic", 
          "VT", "allele", "effect", "impact", "gene", 
          "feature", "biotype", "exon_rank", "hgvs_cdna", "hgvs_protein", 
          "cds_pos", "cds_len", "AA_pos", "AA_len", "type")
                         
keeps <- c("sample", "chr", "pos", "external_id", "ref", "alt", "filter", 
           "depth_normal", "normal.var_depth", "allele_freq_normal", 
           "depth_tumor", "tumor.var_depth", "tumor.base_qual", 
           "allele_freq_tumor", "gt_tumor", "somatic_status", "somatic", 
           "effect", "impact", "gene", "biotype", "hgvs_cdna", "hgvs_protein", "type")
...
# similarly create a normal_id column
mutect.snvs$normal_id <- gsub("(.*)\\_(.*)","\\2", mutect.snvs$sample)
...
tumor_id.to.mappable_tumor_id <- c("BCC20-FFT"="BCC20",
                           "GZ-170-merged.sorted"="GZ-170")
mutect.snvs$tumor_id <- revalue(mutect.snvs$tumor_id, tumor_id.to.mappable_tumor_id)

# remove excluded samples
mutect.snvs <- filter(mutect.snvs, !(mutect.snvs$tumor_id %in% samples.to.exclude))

# remove non-informtive / redundant columns
keeps <- c("tumor_id", "normal_id", "type", "chr", "pos", "gene", "ref", "alt", "hgvs_cdna", "hgvs_protein",
           "depth_normal", "normal.var_depth", "allele_freq_normal",
           "depth_tumor", "tumor.var_depth", "allele_freq_tumor", "gt_tumor", "tumor.base_qual",
...
mutect.snvs <- merge(mutect.snvs, sample.data, by = "tumor_id", all.x = TRUE)
...
mutect.snvs <- filter(mutect.snvs, 
                        (normal.extraction_method == "LMD" & as.numeric(allele_freq_normal) < MAX_VAR_AF_NORMAL.LMD_OPT)
                         | (normal.extraction_method != "LMD" | is.na(normal.extraction_method)))
...
mutect.not_pooledN <- filter(mutect.snvs, normal_id != "pooledN") # dataset 1
mutect.pooledN <- filter(mutect.snvs, normal_id == "pooledN")
...
#   "GEN.NORMAL..DP"   - depth in normal sample
#   "GEN.NORMAL..FDP"
#   "GEN.NORMAL..AU" 
#   "GEN.NORMAL..CU"
#   "GEN.NORMAL..GU"
#   "GEN.NORMAL..TU"
#   "GEN.TUMOR..DP"   - depth in tumor sample
#   "GEN.TUMOR..FDP" 
#   "GEN.TUMOR..AU"
#   "GEN.TUMOR..CU"
#   "GEN.TUMOR..GU"
...
# extract somatic variant allele frequencies from strelka snvs

#strelka.snvs$refCounts <- 

# extract only columns of interest	
strelka.indels <- strelka.indels[, c("SAMPLE", "CHROM", "POS", "REF", "ALT", "type")]
strelka.snvs <- strelka.snvs[, c("SAMPLE", "CHROM", "POS", "REF", "ALT", "type")]
strelka.calls <- rbind(strelka.indels, strelka.snvs)
rm(strelka.indels)	
rm(strelka.snvs)

# create a tumor_id column based on the sample id (format is <tumor_id>_<normal_id>) - e.g. GE0556B_GE0556-N
strelka.calls$tumor_id <- gsub("(.*)\\_(.*)","\\1", strelka.calls$SAMPLE)
# similarly create a normal_id column
strelka.calls$normal_id <- gsub("(.*)\\_(.*)","\\2", strelka.calls$SAMPLE)
...
# remove excluded samples
strelka.calls <- filter(strelka.calls, !(strelka.calls$tumor_id %in% samples.to.exclude))

# rename columns	
colnames(strelka.calls) <- c("sample", "chr", "pos", "ref", "alt", "type", "tumor_id", "normal_id")	

keeps <- c("tumor_id", "normal_id", "type", "chr", "pos", "ref", "alt")
strelka.calls <- strelka.calls[, keeps]
