...
samples <- read.table("/Volumes/shung/projects/gzl_targeted/blood_vs_pool_analysis/sample_pairs.txt", sep = "\t", header = FALSE)
...
## Special filters depending on whether the normal is pooled
...
## Germline filtering by removing variants with VAF between 40-60% and > 90%
...
combine.T_v_N.T_v_pooledN <- intersect(unique(calls.T_v_N$combine), unique(calls.T_v_pooledN$combine)) # 0
combine.T_v_N.N_v_pooledN <- intersect(unique(calls.T_v_N$combine), unique(calls.N_v_pooledN$combine)) # 1133
combine.T_v_pooledN.N_v_pooledN <- intersect(unique(calls.T_v_pooledN$combine), unique(calls.N_v_pooledN$combine)) # 0
...
calls.T_v_pooledN.analyses <- c("T vs pooledN")
