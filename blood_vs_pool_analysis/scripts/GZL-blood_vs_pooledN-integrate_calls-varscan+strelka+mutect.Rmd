...
calls.all.germline_VAF_filtered.COSM_only_id <- rbind(calls.v_matchedN,
                                                      calls.v_pooledN.germline_VAF_filtered,
...
write.table(calls.union, "/Volumes/shung/projects/gzl_targeted/blood_vs_pool_analysis/calls.union/calls.union-ROC_thresholds+germline_VAF_40_to_60_filtered.txt", sep = "\t", quote = FALSE, row.names = FALSE)
write.table(calls.union, "/Volumes/shung/projects/gzl_targeted/blood_vs_pool_analysis/calls.union/calls.union-ROC_thresholds+germline_VAF_40_to_60_filtered+keep_COSM_id.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
## Boxplot distribution of tumor VAF by gene for variants unique to T vs N (not found by T vs pooledN)

```{r}
calls.T_v_N.unique <- filter(calls.union, calls.union$analyses == "T vs N")

p.box <- ggplot(calls.T_v_N.unique, 
             aes(x=gene, y=as.numeric(tumor.allele_freq))) + 
  geom_boxplot() +
  xlab("") +
  ylab("Tumor VAF (%)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
## Boxplot distribution of T vs. pooledN not filtered out by germline filters

```{r}
pooled_datasets <- c("T vs pooledN", "T vs pooledN, N vs pooledN")
calls.T_v_pooledN.only <- filter(calls.union, calls.union$analyses %in% pooled_datasets)

p.box <- ggplot(calls.T_v_pooledN.only, 
             aes(x=gene, y=as.numeric(tumor.allele_freq))) + 
  geom_boxplot() +
  xlab("") +
  ylab("Tumor VAF (%)") +
