...
calls.ffpe <- read.table("/Volumes/shung/projects/gzl_targeted/calls.all/calls.all.unique-union-varscan_mutect_strelka.txt", sep = "\t", header = TRUE, fill = TRUE)
...
calls.ffpe <- merge(calls.ffpe, target.id_mappings, by = "tumor.id")
...
gold.data <- filter(gold.data, gold.data$case_id %in% samples.ffpe$V1)
calls.ffpe <- filter(calls.ffpe, calls.ffpe$case_id %in% samples.ffpe$V1)
...
calls.ffpe <- filter(calls.ffpe, !(calls.ffpe$effect %in% target.UTR))
...
calls.ffpe <- filter(calls.ffpe, calls.ffpe$gene %in% genes.panel)
...
calls.ffpe <- calls.ffpe[, target.keeps]
...
calls.ffpe$key <- as.character(paste(
  calls.ffpe$case_id,
  calls.ffpe$chr,
  calls.ffpe$pos,
  calls.ffpe$ref,
...
write.table(gold.data, "/Volumes/shung/projects/gzl_targeted/roc_analysis/data/gold.data.ffpe.txt", sep = "\t", quote = FALSE, row.names = FALSE)
write.table(calls.ffpe, "/Volumes/shung/projects/gzl_targeted/roc_analysis/data/calls.ffpe.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
gold.data <- read.table("/Volumes/shung/projects/gzl_targeted/roc_analysis/data/gold.data.ffpe.txt", sep = "\t", header = TRUE)
gold.data.intersect <- read.table("/Volumes/shung/projects/gzl_targeted/roc_analysis/data/gold.ffpe.intersect_both_bait_sets.bed", sep = "\t", header = FALSE)
...
calls.ffpe <- read.table("/Volumes/shung/projects/gzl_targeted/roc_analysis/data/calls.ffpe.txt", sep = "\t", header = TRUE)
calls.ffpe.intersect <- read.table("/Volumes/shung/projects/gzl_targeted/roc_analysis/data/calls.ffpe.intersect_both_bait_sets.bed", sep = "\t", header = FALSE)
...
calls.ffpe.intersect$V3 <- NULL
colnames(calls.ffpe.intersect) <- c("chr", "pos")
calls.ffpe.intersect$key <- as.character(paste(
  calls.ffpe.intersect$chr,
  calls.ffpe.intersect$pos,
  sep = "."
))
overlapping_keys <- intersect(unique(calls.ffpe$key), unique(calls.ffpe.intersect$key))
calls.ffpe <- unique(filter(calls.ffpe, calls.ffpe$key %in% overlapping_keys))
...
for (tumor_VAF.threshold_value in tumor_VAF.thresholds) {
  for (tumor_var_reads.threshold_value in tumor_var_reads.thresholds) {
    ...
    calls.filtered <- subset(calls.ffpe, as.numeric(calls.ffpe$tumor.allele_freq) > tumor_VAF.threshold_value & 
                               as.numeric(calls.ffpe$tumor.var_reads) > tumor_var_reads.threshold_value) 
    calls.unfiltered <- subset(calls.ffpe, calls.ffpe$tumor.allele_freq <= tumor_VAF.threshold_value |
                                 calls.ffpe$tumor.var_reads <= tumor_var_reads.threshold_value)
...
write.table(ROC.data, "/Volumes/shung/projects/gzl_targeted/roc_analysis/results/ROC_results-FFPE-tumor_VAF_and_tumor_var_reads.txt", sep = "\t", row.names = FALSE, quote = FALSE)
```
