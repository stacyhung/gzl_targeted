...
# apply different filters depending on tumor content: <= 20% (and pooled normal)  or >20% 
varScan.calls <- filter(varScan.calls, 
                        ( (as.numeric(tumor_content) <= 20 & normal.extraction_method %in% normals.pooled) &
...
write.table(varScan.calls, "/Volumes/shung/projects/gzl_targeted/all_samples/varscan/varscan.calls.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
mutect.snvs$somatic_p_value <- NULL # no longer needed
...
strelka.calls$somatic_p_value <- NULL # no longer needed
...
effects.exclude.more <- c("structural_interaction_variant", "sequence_feature")
calls.all <- filter(calls.all, !(calls.all$effect %in% effects.exclude.more))
calls.all$hgvs_cdna <- NULL
calls.all$hgvs_protein <- NULL
