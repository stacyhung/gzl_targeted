...
target.snpEff_to_exome.snpEff <- c("disruptive_inframe_deletion"="CODON_CHANGE_PLUS_CODON_DELETION",
                                   "disruptive_inframe_insertion"="CODON_CHANGE_PLUS_CODON_INSERTION",
...
```{r}
# if starting this step from scratch:
calls.target.to_integrate <- read.table("/Volumes/shung/projects/gzl_targeted/calls.all/calls.all-target_with_mapped_snpEff.to_integrate.txt", sep = "\t", header = TRUE)
...
# read in case id mappings 
case_id.mappings <- read.table("/Volumes/shung/projects/gzl_exomes/data/tumor_id.to.case_id.mappings.txt", sep = "\t", header = TRUE)

# merge calls with id mappings
exome.calls <- merge(exome.calls, case_id.mappings, by = "tumor_id", all.x = TRUE)

# tumor_id is deprecated; replace with case_id
exome.calls$tumor_id <- NULL;
...
tumor_id.to.mappable_tumor_id <- c("BCC20-FFT"="BCC20",
                                   "GZ-056-FFPE-FS"="GZ-056",
                                   "GZ-125-FFPE-FS"="GZ-125",
                                   "GZ-159-Macro-FFPE"="GZ-159",
                                   "GZ-162-Macro-FFPE"="GZ-162",
                                   "GZ-170-merged"="GZ-170",
                                   "GZ-194-Macro-FFPE"="GZ-194",
                                   "GZ-246-Macro-FFPE"="GZ-246",
                                   "GZ-254-FFPE-FS"="GZ-254",
                                   "GZ-255-FFPE-FS"="GZ-255",
                                   "GZ-256-macro-FFPE"="GZ-256")
calls.all$tumor.id <- revalue(calls.all$tumor.id, tumor_id.to.mappable_tumor_id)

# make sure MLL* genes are mapped to KMT* equivalents
mll_genes.to.kmt_genes <- c("MLL"="KMT2A", "MLL2"="KMT2C", "MLL3"="KMT2D")
calls.all$gene <- revalue(calls.all$gene, mll_genes.to.kmt_genes)
...
# create version of calls that are only for panel genes
genes.panel <- read.table("/Volumes/shung/projects/gzl_exomes/data/target_panel_Agilent_genes.txt", header=FALSE)
genes.panel <- as.vector(genes.panel$V1)
calls.all <- filter(calls.all, calls.all$gene %in% genes.panel)
write.table(calls.all, "/Volumes/shung/projects/gzl_targeted/target+exome/calls.all-exome+target-snvs_indels-no_UTR-panel_genes.txt", sep = "\t", quote = FALSE, row.names = FALSE)
