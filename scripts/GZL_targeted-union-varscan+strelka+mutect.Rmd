...
colnames(snvs) <- c("sample", "chr", "pos", "external_id", "ref", "alt", 
                             ...
                             "gpv", "LOF.temp", "somatic_p_value", "allele", "effect", "impact", "gene", "feature",
                    "feature_ID", "biotype",
                             "exon_rank", "hgvs_cdna", "hgvs_protein", "cds_pos", "cds_len", "AA_pos", "AA_len", "LOF_gene",
                    "LOF_geneid", "LOF_num_Tr", "LOF_perc")

# Important filter that hopefully will solve the "duplicates" issue:

#snvs.bad <- unique(snvs[!grepl("ENST", snvs$feature_ID), ])
snvs.good <- unique(snvs[grep("ENST", snvs$feature_ID), ])

write.table(snvs, "/Volumes/shung/projects/gzl_targeted/targeted_batch2_20191011/varscan/GZ-280_GZ-280ME_LOF_test/snvs.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
colnames(varScan.calls) <- c("sample", "chr", "pos", "external_id", "ref", "alt", 
                             ...
                             "normal.allele_freq", "normal.depth4", "tumor.gt", "tumor.gq", "tumor.depth", "tumor.ref_reads",
                             "tumor.var_reads", "tumor.allele_freq", "tumor.depth4", "depth", "somatic", "somatic_status",
                             "gpv", "somatic_p_value", "allele", "effect", "impact", "gene", "feature", "feature_id",
                             "biotype", "exon_rank", "hgvs_cdna", "hgvs_protein", "cds_pos", "cds_len", 
                             "AA_pos", "AA_len", "LOF_gene", "LOF_geneid", "LOF_num_Tr", "LOF_perc", "type")
...
# NEW - to avoid duplicate (and unwanted) effects for the same variant, only keep mutations with an Ensembl transcript ID
...
keeps <- c("tumor.id", "normal.id", "type", "chr", "pos", "gene", "ref", "alt", "hgvs_cdna", "hgvs_protein", 
           ...
           "somatic_p_value", "LOF_gene", "LOF_geneid", "LOF_num_Tr", "LOF_perc")	
...
keeps <- c("tumor.id", "normal.id", "type", "chr", "pos", "gene", "ref", "alt", "hgvs_cdna", "hgvs_protein",
           ...
           "effect", "impact", "biotype", "feature_id", "external_id", 
           "LOF_gene", "LOF_geneid", "LOF_num_Tr", "LOF_perc")
...
#calls.all <- filter(calls.all, calls.all$effect != "sequence_feature")
